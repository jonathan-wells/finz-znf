...
for species in ${!genomes[@]}; do
    ...
    # rm "../../data/gffs/${species}_augustus_finz."
    for suffix in aa codingseq cdsexons; do
        mv "../../data/gffs/${species}_augustus_finz.${suffix}" ../../data/seqs
    done

    runBITACORA_command_line.sh -m full \
        -sp /usr/local/bitacora/Scripts \
        -gp /usr/local/GeMoMa/GeMoMa-1.6.4/GeMoMa-1.6.4.jar \
        -q ../../data/phmms/bitacora_finz_db \
        -g "../../data/seqs/${species}_finz_blocks.fa" \
        -p "../../data/seqs/${species}_augustus_finz.aa" \
        -f "../../data/gffs/${species}_augustus_finz.gff" \
        -n $species
